...
function disableConfirmationStep() {
  ...
  if (confirmationStep) {
    ...
    confirmationStep.parentNode.replaceChild(newConfirmationStep, confirmationStep);
...
function renderOrderSummary() {
  ...
  cart = cart.map(item => {
    ...
    if (!newItem.quantity || isNaN(parseInt(newItem.quantity)) || parseInt(newItem.quantity) < 1) {
      newItem.quantity = 1;
    }
  ...
  cart.forEach(function (product, index) {
    ...
    li.setAttribute("aria-label", `${product.name}, quantity: ${quantity}, price: $${price.toFixed(2)}`);
  ...
  const cartItems = document.querySelectorAll('.checkout-item:not(.empty-cart)');
...
function showSuccessNotification(message = 'Your order has been placed successfully!', callback = null) {
  ...
  if (notification._callbackTimeout) clearTimeout(notification._callbackTimeout);
  ...
  notification._fadeTimeout = setTimeout(() => {
    ...
    notification._callbackTimeout = setTimeout(() => {
      ...
      if (notification._visibleCallback && typeof notification._visibleCallback === 'function') {
        ...
        const visibleDuration = Date.now() - (notification._visibleTimestamp || Date.now());
...
function redirectToConfirmation() {
  ...
  try {
    ...
    // Fallback redirect
    alert('Your order was successful! Click OK to view your order confirmation.');
    window.location.href = '../confirmation/confirm.html';
  }
}
...
function showErrorNotification(message = 'Please fix the errors in your form.') {
...
document.getElementById('checkout-form').addEventListener('submit', function(e) {
  ...
  try {
    ...
    if (!validateCart(cart)) {
      showErrorNotification("Your cart is empty or contains invalid items. Please add items to your cart before checkout.");
      return false;
    }
  } catch (error) {
    console.error("Error validating cart:", error);
    showErrorNotification("There was a problem with your cart. Please try again.");
    return false;
  }
  ...
  const isUsernameValid = validateInUsername(signInUser);
  ...
  if (!paymentMethodSelected) {
    ...
    showErrorNotification("Please select a payment method before placing your order.");
    return false;
  }
  
  // If there are validation errors, show a specific error message
  if (validationErrors.length > 0) {
    showErrorNotification(`Please correct issues with: ${validationErrors.join(', ')}`);
    return false;
  }
  ...
  if (
    ...
    processingIndicator.innerHTML = '<div class="processing-spinner"></div><p>Processing your order, please wait...</p>';
    ...
    try {
      ...
      const orderData = storeOrderData(cart, {
        name: signInUser,
        email: signInEmail,
        phone: phoneInput.value,
        address: fullAddress
      ...
      console.log('Order data stored successfully:', orderData?.orderId || 'Unknown ID');
      ...
      const selectedPayment = document.querySelector('.payment-method[aria-checked="true"]');
      if (selectedPayment) {
        const paymentMethod = selectedPayment.getAttribute('data-payment-method') || selectedPayment.textContent.trim();
      ...
      console.log('Order placed successfully, showing notification and redirecting...');
      ...
      showErrorNotification('There was a problem processing your order. Please try again.');
    }
  }
});
...
window.addEventListener('storage', function(e) {
  console.log('Storage changed:', e.key, 'Old value:', e.oldValue, 'New value:', e.newValue);
});
...
function validateInUsername(signInUser) {
  ...
  if (!check_signInUsername.test(signInUser)) {
    showFieldError("signInUsername", "Username must start with a capital letter and be at least 6 characters long");
    return false;
  }
  return true;
}
...
function validatePhoneNumber() {
  ...
  if (phoneNumber.length < 10) {
    showFieldError("phoneNumber", "Please enter a valid phone number with at least 10 digits");
    return false;
  }
  return true;
}
